...
if (!containerId) {
  console.error('Usage: node delete-container.mjs GTM-XXXXXXX');
  process.exit(1);
}
...
await tm.accounts.containers.delete({ path: target.path });
console.log(`✓ Deleted container ${containerId} (${target.name})`);
