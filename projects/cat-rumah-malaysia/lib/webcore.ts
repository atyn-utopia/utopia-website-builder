// Unified data layer for products and phone numbers.
...
export async function getProducts(): Promise<ProductRow[]> {
  const path =
    `products?select=id,name,slug,description,parent_id,sale_price,rental_price,sort_order,is_active,product_photos(url,alt_text,sort_order)` +
    `&website=eq.${encodeURIComponent(site.domain)}` +
    `&is_active=eq.true` +
...
const FALLBACK_PHONE = site.phone
const FALLBACK_WA_TEXT = site.whatsappText
