...
export async function getProducts(): Promise<{ core: Product[]; additional: Product[] }> {
  ...
  // The scaffold split these on `rental_price !== null`, because on the rental
  // site every core machine had a daily rate and accessories did not. Aqiqah
  // packages are a SALE — `rental_price` is null on every row — so that test put
  // all four packages into `additional`, left `core` empty, and the pages
  // silently rendered the translated offline fallback instead of the live
  // Supabase rows. Split on the schema's own parent/child field instead:
  // top-level products are core, variants hanging off a parent are additional.
  return {
    core: products.filter((p) => p.parent_id === null),
    additional: products.filter((p) => p.parent_id !== null),
  };
}
