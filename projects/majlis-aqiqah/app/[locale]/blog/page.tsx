  ...
  return (
    <>
                ...
                return (
                  <article key={post.id} className="blog-card">
                    {post.cover_image_url && (
                      <BlogLinkTracker slug={post.slug} href={localePath(locale, `/blog/${post.slug}`)} className="blog-card-img">
                    ...
                    <div className="blog-card-body">
                      ...
                      <h3><BlogLinkTracker slug={post.slug} href={localePath(locale, `/blog/${post.slug}`)}>{tr?.title}</BlogLinkTracker></h3>
                      <p className="blog-card-excerpt">{tr?.excerpt}</p>
                      <BlogLinkTracker slug={post.slug} href={localePath(locale, `/blog/${post.slug}`)} className="blog-card-link">{t('readMore')} →</BlogLinkTracker>
                    </div>
                  </article>
                );
              })}
      ...
      <style>{`
        ...
        /* Girih tessellation, matching the dark sections on the rest of the site. */
        .blog-hero-bg::after {
          content: '';
          position: absolute; inset: 0;
        ...
        .breadcrumb a { color: var(--brand-gold-bright); font-weight: 600; }
        ...
        .blog-card-date { font-family: var(--font-display); font-weight: 600; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--ink-muted); margin: 0; }
        .blog-card h3 { font-size: 19px; font-weight: 700; color: var(--brand-emerald); letter-spacing: -0.015em; line-height: 1.3; margin: 0; }
        ...
        .blog-card-link { color: var(--brand-gold-deep); font-weight: 700; font-size: 14px; margin-top: auto; }

        .blog-cta { background: var(--brand-gold-pale); padding: 56px 0; }
        .blog-cta-inner {
        ...
        .blog-cta h3 { font-size: 22px; font-weight: 700; color: var(--brand-emerald); margin: 0 0 6px; letter-spacing: -0.015em; }
