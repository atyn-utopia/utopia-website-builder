...
export default async function BlogPostPage({
  ...
  return (
    <>
      ...
      <BreadcrumbSchema
        items={[
          { name: t('breadcrumbHome'), url: `${localeAbs(locale)}` },
          { name: t('breadcrumbBlog'), url: `${localeAbs(locale)}/blog` },
          { name: tr.title, url: `${localeAbs(locale)}/blog/${slug}` },
        ]}
      />
        ...
        <header className="post-header">
          <div className="container">
            <nav className="breadcrumb" aria-label="Breadcrumb">
              <Link href={localePath(locale)}>{t('breadcrumbHome')}</Link>
              <span aria-hidden="true">/</span>
              <Link href={localePath(locale, '/blog')}>{t('breadcrumbBlog')}</Link>
              <span aria-hidden="true">/</span>
        ...
        <div className="container post-body-wrap">
          <div className="post-body">
            <div
              ...
              dangerouslySetInnerHTML={{ __html: localizeArticleHtml(tr.content, locale) }}
            />
      ...
      <style>{`
        ...
        .post-header .breadcrumb a { color: var(--brand-gold-deep); font-weight: 600; }
        .post-header .breadcrumb [aria-current="page"] { color: var(--brand-emerald); font-weight: 600; }
        .post-header h1 { font-size: clamp(2rem, 4vw, 2.875rem); font-weight: 800; color: var(--brand-emerald); letter-spacing: -0.025em; line-height: 1.1; margin: 0 0 12px; max-width: 800px; }
        .post-excerpt { font-size: clamp(1rem, 1.4vw, 1.1875rem); color: var(--ink-muted); line-height: 1.6; max-width: 740px; margin: 0; }
        .post-meta { display: inline-flex; flex-wrap: wrap; gap: 10px; font-family: var(--font-display); font-weight: 600; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--ink-faint); margin: 18px 0 0; }
        ...
        .blog-cta-banner { margin: 48px 0 0; background: var(--brand-gold-pale); border-radius: var(--radius-card); padding: 32px; text-align: center; border: 1px solid var(--brand-gold-ring); }
        .blog-cta-banner h3 { font-size: 22px; font-weight: 700; color: var(--brand-emerald); margin: 0 0 8px; letter-spacing: -0.015em; }
        ...
        .post-sidebar h4 { font-family: var(--font-display); font-weight: 700; font-size: 11px; letter-spacing: var(--label-tracking); text-transform: uppercase; color: var(--brand-gold-deep); margin: 0 0 12px; }
        ...
        .post-sidebar a:hover { color: var(--brand-gold-deep); }
      `}</style>
    </>
  );
}
