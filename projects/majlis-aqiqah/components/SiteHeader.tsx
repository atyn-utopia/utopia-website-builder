    ...
    <header className="site-header">
      <div className="container site-header-inner">
        <nav className="site-nav site-nav--desktop" aria-label="Primary">
          ...
          <Link href={`${localePath(locale)}#products`}>{t('products')}</Link>
          <Link href={`${localePath(locale)}#calculator`}>{t('calculator')}</Link>
          <Link href={`${localePath(locale)}#locations`}>{t('locations')}</Link>
          <Link href={localePath(locale, '/blog')}>{t('blog')}</Link>
        </nav>
      ...
      <div id="site-nav-mobile" className={`site-mobile-drawer ${open ? 'is-open' : ''}`} aria-hidden={!open}>
        <nav className="site-mobile-nav" aria-label="Mobile primary">
          <Link href={localePath(locale)} onClick={close}>{t('home')}</Link>
          <Link href={`${localePath(locale)}#products`} onClick={close}>{t('products')}</Link>
          <Link href={`${localePath(locale)}#calculator`} onClick={close}>{t('calculator')}</Link>
          <Link href={`${localePath(locale)}#locations`} onClick={close}>{t('locations')}</Link>
          <Link href={localePath(locale, '/blog')} onClick={close}>{t('blog')}</Link>
        </nav>
      ...
      <style jsx>{`
        ...
        .site-nav a:hover { color: var(--brand-gold-deep); }
        ...
        .site-burger span { display: block; height: 2px; width: 100%; background: var(--brand-emerald); border-radius: 2px; transition: transform 0.18s ease, opacity 0.18s ease; }
        ...
        .site-mobile-nav a { padding: 13px 4px; font-weight: 700; font-size: 15px; color: var(--brand-emerald); border-bottom: 1px solid var(--line); }
