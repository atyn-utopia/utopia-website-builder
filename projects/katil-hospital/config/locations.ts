...
/**
 * Nearby map — each location links to 3 real neighbouring cities in the same state
 * (with one optional cross-state neighbour where geography warrants).
 * Used for internal linking on location pages.
 */
function inState(state: string): string[] {
  return locations.filter((l) => l.state === state).map((l) => l.slug);
}
...
export function getNearbyLocations(slug: string): Location[] {
  ...
  // Stable deterministic pick: first 4 peers in the state.
  return peers.slice(0, 4);
}

// Legacy export (still used by older code paths) — empty map is fine because
// getNearbyLocations now derives neighbours dynamically from `locations` above.
export const nearbyMap: Record<string, string[]> = Object.fromEntries(
  locations.map((l) => [
    l.slug,
    locations
      .filter((x) => x.state === l.state && x.slug !== l.slug)
      .slice(0, 4)
      .map((x) => x.slug),
  ])
);
