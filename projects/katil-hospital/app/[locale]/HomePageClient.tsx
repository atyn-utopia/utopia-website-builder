...
/* ── Static data ──
   wallPrice = daily rental rate, cassettePrice = monthly rental rate
   (labelled via services.daily / services.monthly in messages). */
const serviceKeys = [
  { key: 'manualBed', wallPrice: 'RM30', cassettePrice: 'RM150', icon: 'wrench', badgeKey: 'popular', waKey: 'waManualBed' },
  { key: 'electricBed', wallPrice: 'RM50', cassettePrice: 'RM280', icon: 'bolt', badgeKey: 'recommended', waKey: 'waElectricBed' },
  { key: 'semiElectricBed', wallPrice: 'RM40', cassettePrice: 'RM220', icon: 'gauge', badgeKey: null, waKey: 'waSemiElectricBed' },
  { key: 'rippleMattress', wallPrice: 'RM15', cassettePrice: 'RM80', icon: 'droplet', badgeKey: null, waKey: 'waRippleMattress' },
  { key: 'wheelchair', wallPrice: 'RM15', cassettePrice: 'RM90', icon: 'unplug', badgeKey: null, waKey: 'waWheelchair' },
  { key: 'oxygenConcentrator', wallPrice: 'RM40', cassettePrice: 'RM250', icon: 'drill', badgeKey: null, waKey: 'waOxygenConcentrator' },
]

const whyKeys = ['certified', 'sameDay', 'transparent', 'allBrands', 'warranty', 'open7Days']

const brands = [
  { name: 'Hopkin', logo: '/images/brands/hopkin.svg' },
  { name: 'Hillrom', logo: '/images/brands/hillrom.svg' },
  { name: 'Paramount', logo: '/images/brands/paramount.svg' },
  { name: 'Stryker', logo: '/images/brands/stryker.svg' },
  { name: 'Invacare', logo: '/images/brands/invacare.svg' },
]
      ...
      <div style={{ maxHeight: open ? '600px' : '0px', overflow: 'hidden', transition: 'max-height 0.35s ease' }}>
        <div className="px-5 pb-5 pt-1">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {cities.map(city => (
              <a key={city.slug} href={`/${locale}/sewa-katil-hospital/${city.slug}`} className="text-xs font-normal px-3 py-1.5 rounded-lg hover:opacity-80 transition-opacity" style={{ background: 'var(--brand-blue-xs)', color: 'var(--brand-navy)' }}>
                {city.name}
              </a>
            ))}
          </div>
