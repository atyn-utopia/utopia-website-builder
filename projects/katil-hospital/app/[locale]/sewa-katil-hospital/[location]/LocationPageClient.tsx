...
export default function LocationPageClient({ locale, locationSlug, cityName, phoneNumber, nearby }: Props) {
  ...
  const waLink = waRedirect(locale, `Hi, saya berminat untuk sewa katil hospital di ${cityName}`, locationSlug)

  const serviceItems = [
    { key: 'manualBed', icon: '🛏' },
    { key: 'electricBed', icon: '⚡' },
    { key: 'rippleMattress', icon: '🪶' },
    { key: 'delivery', icon: '🚚' },
  ]
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <FadeSection>
                <div className="flex flex-wrap justify-center gap-3">
                  {nearby.map(n => (
                    <a key={n.slug} href={`/${locale}/sewa-katil-hospital/${n.slug}`} className="px-5 py-3 rounded-xl text-sm font-semibold transition-opacity hover:opacity-80" style={{ background: 'var(--brand-blue-xs)', color: 'var(--brand-navy)', border: '1px solid var(--brand-border)' }}>
