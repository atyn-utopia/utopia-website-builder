...
/**
 * Each flag needs its own clipPath id — sharing one id across the three flags
 * makes every flag after the first render as a square.
 */
...
export default function LanguageSwitcher() {
  ...
  const href = (l: string) => `/${l}${rest}${suffix}`;

  return (
    <div className="lsw" ref={rootRef}>
      {/* Desktop: three standalone pills side by side */}
      <div className="lsw-toggle" role="group" aria-label="Tukar bahasa">
      ...
      {/* Mobile: collapsed trigger + dropdown */}
      <button
        type="button"
      ...
      <div className={`lsw-menu ${open ? 'is-open' : ''}`} role="menu">
        {routing.locales.map((l) => {
          const active = l === currentLocale;
          return (
            <Link
