...
export async function POST(request: NextRequest) {
  if (!SECRET) {
    return NextResponse.json({ error: 'WEBCORE_REVALIDATE_SECRET not set' }, { status: 500 })
  }
  if (request.headers.get('x-webcore-secret') !== SECRET) {
  ...
  for (const tag of body.tags) {
    if (typeof tag === 'string' && ALLOWED_TAGS.has(tag)) {
      // Next 16 requires the second arg ('max' = full purge).
