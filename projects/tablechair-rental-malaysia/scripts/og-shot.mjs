...
const LOCALES = [
  // defaultLocale here is EN (i18n/routing.ts), so English is served from the
  // bare root and ms/zh carry the prefix — the reverse of the ms-default sites.
  ['en', ''],
  ['ms', '/ms'],
  ['zh', '/zh'],
];
const PORT = 3000;   // package.json `start` is a bare `next start`
// The hero is a Tailwind-classed section (relative overflow-hidden) with no
