...
// The hero is a Tailwind-classed section inside HomePageClient with no `hero`
// class, so the template's default selector matches nothing here. It does
// carry aria-label="Hero", which is stable and unique.
const HERO = 'section[aria-label="Hero"]';
const HIDE = [
  '.fomo-bar', '.site-header', 'header',   // canonical chrome
  '.hero-stats', '.ops-ticker',            // hero-foot elements a crop would slice
  // This hero is 857px — well past the 630 box — so a top-anchored clip cut the
  // sub-copy mid-sentence and lost the CTA entirely. Dropping the logo block
  // (the H1 already carries the brand name) brings the CTA back inside the card.
  '.hero-logo',
  '.fixed.bottom-6',                       // floating WhatsApp FAB — a UI artifact on a static card
];
// ─────────────────────────────────────────────────────────────────────────
