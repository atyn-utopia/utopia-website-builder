...
// The hero is a Tailwind-classed section with no `hero` class, so the
// template's default selector matches nothing here. Match on the arbitrary
// min-h-[90vh] utility via a substring attribute selector — writing it as a
// class selector would need the brackets CSS-escaped.
const HERO = 'section[class*="min-h-[90vh]"]';
const HIDE = [
  '.fomo-bar', '.site-header', 'header',   // canonical chrome
  // This site has no .fomo-bar class: the FOMO banner and the header share one
  // `sticky top-0 z-50` wrapper in [locale]/layout.tsx. Hiding that wrapper
  // drops both — and it must go, because the banner's live countdown would be
  // frozen at whatever the clock read when the screenshot was taken.
  '.sticky.top-0.z-50',
