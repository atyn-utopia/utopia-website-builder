...
export default async function MarketingMarquee({
  ...
  // Build one "half" that is at least ~24 items wide (so it exceeds any viewport),
  // then duplicate that half. translateX(-50%) then loops perfectly seamlessly with
  // no blank gap — the earlier single-duplicate left a gap on wide screens.
  const half: string[] = [];
  while (half.length < Math.max(24, items.length * 3)) half.push(...items);
