...
const LOCALES = [
  // This site's defaultLocale is EN (config/site.ts + i18n/routing.ts), so
  // English is served from the bare root and ms/zh carry the prefix — the
  // reverse of the ms-default sites in the fleet.
  ['en', ''],
  ['ms', '/ms'],
  ['zh', '/zh'],
];
const PORT = 3011;
const HIDE = [
  '.fomo-bar', '.site-header', 'header',   // canonical chrome (nav is .nav-wrap on a <header>)
  '.hero-stats', '.ops-ticker',            // hero-foot elements a crop would slice
  // .usp-bar sits at margin-top:-40px, so it overlaps the hero's own box and
  // lands inside the element screenshot as a strip of half-cut USP text.
  '.usp-bar',
  '.fab-wa',                               // floating WhatsApp button — reads as a UI artifact on a static card
];
// ─────────────────────────────────────────────────────────────────────────
