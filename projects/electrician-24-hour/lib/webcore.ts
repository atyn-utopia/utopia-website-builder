 ...
 * Blog
 *
 * This site exposes a flattened BlogPost shape (translation fields
 * collapsed onto the post) — preserved from the original
 * lib/getBlogPosts.ts so existing consumers keep working.
 * ============================================================ */
...
export async function getBlogPosts(language: string = 'en'): Promise<BlogPost[]> {
  const path =
    `blog_posts?select=id,slug,cover_image_url,published_at,created_at,` +
  ...
  const rows = await webcoreFetch<BlogPostRow[]>(path, 'webcore-blog');
  if (!rows) return [];
