...
export async function getProducts(): Promise<Product[]> {
  ...
  return rows.map((r) => {
    const rawPhotos = r.product_photos;
...
interface PhoneRow {
  ...
  // Nominates the number the chrome PRINTS. Unique per (website, page_slug),
...
// Same host the fleet's lib/webcore.ts files use; it 307s to the new webcore
// host. Moved fleet-wide or not at all — see docs/webcore-api.md.
...
function mapBlogRow(row: BlogRow): BlogPost {
  const t = row.blog_translations?.[0] ?? {};
  return {
    id: row.id,
    slug: row.slug,
    cover_image_url: row.cover_image_url ?? '',
    published_at: (row.published_at || row.created_at || '') as string,
    title: t.title ?? '',
    content: t.content ?? '',
    excerpt: t.excerpt ?? '',
...
export async function getBlogPosts(language: string = 'ms'): Promise<BlogPost[]> {
  ...
  const rows = await webcoreFetch<BlogRow[]>(path, 'webcore-blog');
  if (!rows) return [];
