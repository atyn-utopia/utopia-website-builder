...
export default function Calculator({
  ...
  rates: { ec200: number; ec400: number };
}) {
    ...
    <div className="calc-panel">
          ...
          <div className="calc-segmented">
            {(['ec200', 'ec400'] as Model[]).map((m) => (
              <button
                key={m}
                type="button"
                className={`calc-chip ${model === m ? 'is-active' : ''}`}
                onClick={() => setModel(m)}
              >
                {t(`models.${m}`)}
              </button>
          ...
          <div className="calc-segmented">
            {(['daily', 'weekly', 'monthly'] as Period[]).map((p) => (
              <button
                key={p}
                type="button"
                className={`calc-chip ${period === p ? 'is-active' : ''}`}
                onClick={() => setPeriod(p)}
              >
                {t(`periods.${p}`)}
              </button>
      ...
      <style jsx>{`
        ...
        .calc-glow {
          ...
          background: radial-gradient(40% 60% at 20% 20%, rgba(242,108,31,0.20) 0%, rgba(242,108,31,0) 70%);
          pointer-events: none;
        }
        .calc-grid {
          display: grid;
