...
async function webcoreFetch<T>(path: string, tag: WebcoreTag): Promise<T | null> {
  ...
  try {
    ...
    if (!res.ok) {
      // eslint-disable-next-line no-console
      console.error(`[webcore] ${tag} ${res.status} :: ${path}`)
      return null
    }
...
function fallbackResult(): PhoneResult {
  return { phone: FALLBACK_PHONE, whatsappText: FALLBACK_WA_TEXT_ATTRIBUTED, source: 'fallback', mode: 'fallback' }
}
...
export async function getPhoneNumber(
  ...
  try {
    ...
    switch (mode) {
      case 'single': return toResult(defaultRow ?? rows[0], mode, domain)
      case 'rotation': return toResult(pickWeighted(rows), mode, domain)
      case 'location': {
        if (locationSlug) {
      ...
      default: return toResult(defaultRow, mode, domain)
    }
  } catch (err) {
    // eslint-disable-next-line no-console
...
export async function getWhatsAppLink(locationSlug?: string, messageOverride?: string): Promise<string> {
...
interface BlogPostRow {
  ...
  blog_translations:
    | { title: string | null; content: string | null; excerpt: string | null; meta_title: string | null; meta_description: string | null }[]
    | null
}
