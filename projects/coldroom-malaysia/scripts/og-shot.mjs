...
const LOCALES = [
  // defaultLocale here is EN, so English is served from the bare root and ms/zh
  // carry the prefix — the reverse of the ms-default sites in the fleet.
  ['en', ''],
  ['ms', '/ms'],
  ['zh', '/zh'],
];
const PORT = 3009;
// The hero on this site is a <header> element (not a <section>), so the
// template's `section.hero` selector matches nothing. It uniquely contains
// .hero-copy.
const HERO = 'header:has(.hero-copy)';
const HIDE = [
  // NOTE: the bare 'header' of the canonical list is deliberately NOT here.
  // This site's HERO is itself a <header>, so hiding `header` would blank the
  // very element being screenshotted. The nav is .site-nav, which is enough.
  '.fomo-bar', '.site-header', '.site-nav',
  '.hero-stats', '.ops-ticker',            // hero-foot elements a crop would slice
  '.wa-fab',                               // floating WhatsApp FAB — a UI artifact on a static card
];
// ─────────────────────────────────────────────────────────────────────────
